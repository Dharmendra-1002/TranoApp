...
const FinanceAddAmountTable = () => {
  ...
  const collapsedTableData = fullTableData.map(row => [row[0],row[4], row[6]]);
    ...
    <SafeAreaView style={{ flex: 1, alignItems: 'center', backgroundColor: '#4e2d87' }}>
      <View style={{ backgroundColor: '#fff', width: '92%', height: '95%', borderRadius: 10, marginTop: 20 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('FinanceAddNewForm')}>
            <View>
...
export default FinanceAddAmountTable;
