...
const MarketingExpenseTable = () => {
  ...
  const fullTableData = [
    ...
    ['02', 'Tripti kumari', '       Telangana', '21-2-22', 'TRANS001', 'Traveling', '   40000'],
    // Additional rows...
  ];

  const collapsedTableData = fullTableData.map(row => [row[1],row[2], row[6]]);
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'column' }}>
            <Text style={{ textDecorationLine: 'underline', fontSize: 18, fontWeight: 'bold', marginLeft: 20, marginTop: 20 }}>
              Marketing Head Expense
            </Text>
          </View>
          <TouchableOpacity onPress={() => navigation.navigate('MarketingExecutiveExpense_Form')}>
            <View>
